...
function WeightChart({ data }) {
  ...
  while (i < data.length - 1) {
    ...
    // segment from i to j (inclusive for continuity)
    const segData = data.slice(i, Math.min(j + 1, data.length))
    const points = segData.map((d, k) => `${xPos(i + k)},${yPos(d.weight)}`).join(' ')
    segments.push({ phase, points })
    i = j
  }
  ...
  function handleMouseMove(e) {
    ...
    setTooltip({
      ...
      color: PHASE_COLORS[d.phase_type] || '#888888',
    })
  }
  ...
  return (
    <div className="bg-[#141414] border border-[#333333] p-3 relative">
      ...
      >
        ...
        {segments.map((seg, i) => (
          <polyline
            ...
            points={seg.points}
            fill="none"
            stroke={PHASE_COLORS[seg.phase] || '#888888'}
            strokeWidth="2"
      ...
      {/* Legend */}
      {[...new Set(data.map(d => d.phase_type))].filter(p => p !== 'unknown').length > 1 && (
        <div className="flex gap-4 mt-2 justify-center">
          {[...new Set(data.map(d => d.phase_type))].filter(p => p !== 'unknown').map(phase => (
...
export default function WeightHistory({ onNavigate }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [weightData, phaseData] = await Promise.all([getWeightsWithPhase(), getActivePhase()])
  ...
  const chartData = [...filtered]
    ...
    .map(w => ({
      ...
      phase_type: w.phase_type || 'unknown',
    }))
