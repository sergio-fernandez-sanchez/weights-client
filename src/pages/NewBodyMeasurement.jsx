...
import { postBodyMeasurement } from '../api/client'
...
const METRICS = [
  ...
  { key: 'bicep_cm',     label: 'BÍCEP (cm)' },
  { key: 'waist_cm',     label: 'CINTURA (cm)' },
  { key: 'hip_cm',       label: 'CADERA (cm)' },
  { key: 'thigh_cm',     label: 'MUSLO (cm)' },
]

export default function NewBodyMeasurement({ onNavigate }) {
  const [dateMode, setDateMode] = useState('today')
  const [manualDate, setManualDate] = useState('')
  const [values, setValues] = useState(Object.fromEntries(METRICS.map(m => [m.key, ''])))
  ...
  async function handleSubmit(e) {
    ...
    try {
      const data = Object.fromEntries(
        METRICS.map(m => [m.key, values[m.key] ? parseFloat(values[m.key].replace(',', '.')) : null])
      )
      if (dateMode === 'manual' && manualDate) data.date = parseManualDate(manualDate)
      await postBodyMeasurement(data)
      setToast({ msg: '✓  medidas guardadas', type: 'success' })
    ...
    <div className="min-h-screen px-6 md:px-16 pb-10">
      <div className="w-full max-w-sm mx-auto pt-10">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          {METRICS.map(m => (
            <Input key={m.key} label={m.label} type="number" step="0.1" value={values[m.key]} onChange={e => handleChange(m.key, e.target.value)} />
          ))}
          <div className="mt-2">
