export default function Card({ children, className = '', accent = null, label = null, glow = false }) {
  return (
    <div
      className={`relative glass-card glass-sheen rounded-sm p-4 transition-all duration-300 overflow-hidden ${glow ? 'glass-inner-glow' : ''} ${className}`}
      style={accent ? { borderColor: `${accent}20` } : undefined}
    >
      {accent && (
      ...
      {label && (
        <div className="flex items-center gap-2.5 mb-3">
          ...
          <span className="text-[#666666] font-sans text-[10px] tracking-[0.2em] uppercase">{label}</span>
        </div>
      )}
      {children}
    </div>
